...
void
DaphneApplication::generate_modules(std::shared_ptr<appmodel::ConfigurationHelper> helper) const
{
  ...
  for (auto d2d_conn : get_detector_connections()) {
    ...
    // A ExcludableEntity can be disabled and still its application can be enabled because the application can have
    // multile resources, so we need to check which resources are enabled
    ...
    if (flx_conn) {
      ...
      for (const auto* felix_sender : det_senders) {
        ...
        if (helper->is_excluded(felix_sender)) {
    ...
    if (net_conn) {
      ...
      for (const auto* nw_sender : det_senders) {
        if (helper->is_excluded(nw_sender)) {
  ...
  for (const auto& [id, v3] : v3_map) {
    ...
    conffwk::ConfigObject module_obj =
      obj_fac.create((v3 ? "DaphneV3ControllerModule" : "DaphneV2ControllerModule"), fmt::format("controller-{}", id));
    ...
    if (v3) {
      ...
      hermes_obj.set_by_val<std::string>("uri",
                                         fmt::format("{}://{}:{}",
