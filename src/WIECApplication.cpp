  ...
  for (auto d2d_conn : get_detector_connections()) {
    ...
    if (!nw_receiver) {
      throw(BadConf(ERS_HERE,
                    fmt::format("WEICApplication requires NWDetDataReceiver, found {} of class {}",
                                det_receiver->UID(),
    ...
    // For the same reason, in this code, we create a map of control hosts to senders, without checking if the senders
    // are excluded. Once the map is created, if all the senders associated to a control hosts are excluded, then we
    // skip the creation of all related modules.
    ...
    for (const auto& [ctrlhost, senders] : ctrlhost_sender_map) {
      ...
      if (this->get_wib_module_conf()) {
        ...
        wib_obj.set_by_val<std::string>("wib_addr",
                                        fmt::format("{}://{}:{}",
                                                    this->get_wib_module_conf()->get_communication_type(),
                                                    ctrlhost,
